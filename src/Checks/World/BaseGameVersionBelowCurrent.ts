...
export default class BaseGameVersionBelowCurrent extends Check {
    readonly definition: CheckDefinition = {
        group: WorldChecks.GROUP,
        number: WorldChecks.BASE_GAME_VERSION_BELOW_CURRENT,
        slug: "base-game-version-below-current",
        severity: "recommendation",
        description: "base_game_version older than current release",
    };
    ...
    async run(context: CheckContext): Promise<Finding[]> {
        ...
        for (const pack of context.model.packs) {
            ...
            if (version === undefined || !VersionUtilities.isBelowCurrent(version, current)) {
                continue;
            }
